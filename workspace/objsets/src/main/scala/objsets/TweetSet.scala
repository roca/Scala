...
class Empty extends TweetSet {
  ...
  def asTweetList: TweetList = Nil

  def reverse(tl: TweetList): TweetList = Nil
...
*/
  ...
  def mostRetweeted : Tweet = {
    ...
    def f(remaining: TweetList, t: Tweet): Tweet= {
      if (remaining.isEmpty) t
      else if ((remaining.head.retweets > t.retweets)) f(remaining.tail, remaining.head)
      else f(remaining.tail, t)
    }

    f(this.asTweetList, this.asTweetList.head)
  }

   def descendingByRetweet: TweetList = {

    def f(remaining: TweetSet, tl: TweetList): TweetList= {
      if (remaining.asTweetList.isEmpty) tl
      else {
        val most_retweeted = remaining.mostRetweeted
        f(remaining.remove(most_retweeted), new Cons(most_retweeted,tl))
      }
    }
