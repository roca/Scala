...
object Huffman {
  ...
  def until(singleton_funct: List[CodeTree] => Boolean, 
            combine_funct  : List[CodeTree] => List[CodeTree])
            (trees: List[CodeTree]): List[CodeTree]  = {
    if (singleton_funct(trees)) trees
    else until(singleton_funct,combine_funct) (combine_funct(trees))
  }
  ...
  def codeBits2(CodeTable: CodeTable)(char: Char): List[Bit] = {
    val x = CodeTable.find( p => p match {
    					case (c,list) => c == char
    				}
    		) 
    x match {
  ...
  def convert(tree: CodeTree): CodeTable = {
    def f(tree: CodeTree,acc: List[Bit]): CodeTable = {
    	tree match {
    		   case Leaf( char, weight) => List((char,acc.reverse))
    		   case Fork(left, right, chars, weight) =>  mergeCodeTables(f(left,0::acc), f(right,1::acc))
      }
    }
    f(tree,List()).reverse
  } 
